...
var worker = {
  ...
  _grasp: function(url, http_protocol, page_cb) {
    http_protocol.get(url, function(res) {
      ...
      // retry for some kinds of request error
      var err_should_retry = ["socket hang up", "connect ETIMEDOUT"];
      for (var i = 0; i < err_should_retry.length; i++) {
        if (e.message.indexOf(err_should_retry[i]) > -1) {
          debug("retry action for http request error when getting %s: %s", url, e.message);
          worker._grasp(url, http_protocol, page_cb);
          return;
        }
      };
